  ...
  const results = await Promise.all(
    sessions.map(async (session) => {
      ...
      const terms = await Term.findAll({
        where: { session_id },
        attributes: ["term_id", "name", "start_date", "end_date", "is_active"],
...
export const getUserSessionsAndTerms = async (
  ...
  const prioritizeActive = (
    sessions: any[],
    sessionMap: Record<string, Set<string>>,
    terms: any[]
  ) => {
    const result = sessions.map((session) => {
      const sessionTerms = terms
        .filter((term) => sessionMap[session.session_id!]?.has(term.term_id))
        .map((term) => ({
          term_id: term.term_id,
          name: term.name,
          is_active: term.term_id === activeTerm?.term_id,
        }));

      // Sort terms by active status (active first)
      sessionTerms.sort((a, b) => Number(b.is_active) - Number(a.is_active));

      return {
        session_id: session.session_id,
        session_name: session.name,
        is_active: session.session_id === activeSession?.session_id,
        terms: sessionTerms,
      };
    });

    // Sort sessions by active status (active first)
    result.sort((a, b) => Number(b.is_active) - Number(a.is_active));

    // Clean up is_active before returning
    return result.map(({ is_active, ...session }) => ({
      ...session,
      terms: session.terms.map(({ is_active, ...term }) => term),
    }));
  };
      ...
      return [
        {
          ...
          terms: [{ term_id: activeTerm.term_id, name: activeTerm.name }],
        },
      ];
    }
  ...
  throw new AppError("Only students or teachers can access session-terms", 403);
};
