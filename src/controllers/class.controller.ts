...
export const getAllClassesHandler = async (
  ...
  try {
    if (!school_id || !session_id || !term_id) {
      throw new AppError("school_id, session_id and term_id are required", 400);
    }
...
export const getAllClassNoAuthHandler = async (
  req: Request,
  res: Response
): Promise<void> => {
  const { school_id } = req.params;
  ...
  try {
    ...
    const classes = await classService.getAllClassesOfSchoolNoAuth(school_id);
